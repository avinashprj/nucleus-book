...
export const Post = ({ post }) => {
  ...
  useEffect(() => {
    if (users.length > 0)
      setUserDetails(
        users.filter((user) => user.username === post.username)[0]
      );
  }, [users, post.username]);
  // On Click Functions
  ...
  const bookmarkClickHandler = () => {
    ...
    console.log("🚀 ~ file: Post.jsx ~ line 22 ~ Post ~ authUser", authUser);
  };

  const copyHandler = () => {
    navigator.clipboard.writeText(
      `https://nucleus-book.vercel.app/post/${post._id}`
    );
        ...
        <div className="border-b border-gray-200 dark:border-dim-200 hover:bg-gray-100 dark:hover:bg-dim-300 cursor-pointer transition duration-350 ease-in-out pb-4 border-l border-r">
          ...
          <div className="pl-16">
            <Link to={`/post/${post._id}`}>
              ...
              <div className="flex my-3 mr-2 rounded-2xl border border-gray-200 dark:border-gray-600">
                {/* <img
                className="rounded-2xl"
                src="https://pbs.twimg.com/media/EnTkhz-XYAEH2kY?format=jpg&name=small"
                alt=""
              /> */}
                {post.mediaURL && post.mediaURL.split("/")[4] === "image" ? (
                  <img className="rounded-2xl" src={post.mediaURL} alt="" />
                ) : (
                  post.mediaURL && (
                ...
                <div className="flex items-center">
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center text-gray-800 dark:text-white text-xs text-gray-400 hover:text-red-600 dark:hover:text-red-600 transition duration-350 ease-in-out ${
                      likedByUser() ? "text-red-600 dark:text-red-600" : ""
                    }`}
                  >
                    <svg
                      viewBox="0 0 24 24"
